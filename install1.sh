...
# Interactive choice if no root SSH keys
SHOW_PRIVATE_KEY="no"
ENABLE_PASSWORD_AUTH="no"

if [[ "$ROOT_KEYS_EXIST" == "no" ]]; then
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "⚠ WARNING: No SSH keys found in /root/.ssh/authorized_keys"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""
  echo "Without SSH keys, you won't be able to login after reboot!"
  echo ""
  echo "Choose how to proceed:"
  echo "  1) Show private key to copy (you must save it locally)"
  echo "  2) Enable password authentication (less secure)"
  echo "  3) Abort script - add SSH key to /root first"
  echo ""
  read -r -p "Your choice [1/2/3]: " SSH_CHOICE

  case "$SSH_CHOICE" in
    1)
      SHOW_PRIVATE_KEY="yes"
      ENABLE_PASSWORD_AUTH="no"
      ;;
    2)
      SHOW_PRIVATE_KEY="no"
      ...
      ;;
    3)
      echo "Aborting. Please add your SSH public key to /root/.ssh/authorized_keys"
...
# Show private key if user chose option 1
if [[ "$SHOW_PRIVATE_KEY" == "yes" ]]; then
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  ...
  echo ""
  echo "Instructions:"
  echo "  1. Copy the key above (from -----BEGIN to -----END)"
  echo "  2. Save to ~/.ssh/${NEWUSER}_key on your LOCAL machine"
  echo "  3. Run: chmod 600 ~/.ssh/${NEWUSER}_key"
  echo "  4. Connect: ssh -i ~/.ssh/${NEWUSER}_key -p ${SSH_PORT} ${NEWUSER}@<SERVER_IP>"
  echo ""
  read -r -p "Press ENTER when you have saved the key..."
fi
...
# Conditionally set PasswordAuthentication based on user choice
if [[ "$ENABLE_PASSWORD_AUTH" == "yes" ]]; then
  apply_sshd_conf "PasswordAuthentication" "yes"
  echo "⚠ Password authentication ENABLED (you chose this option)"
else
...
for f in /etc/ssh/sshd_config.d/*.conf; do
  [[ -f "$f" ]] || continue
  if [[ "$ENABLE_PASSWORD_AUTH" == "no" ]]; then
...
# Define display_qr_code function FIRST
display_qr_code() {
  local client_id="$1"
  ...
  rm -f "$tmp"
  echo "  (QR code generation failed - config may be too large or tools missing)"
  return 1
}
...
echo "   Root login:            NO"
if [[ "$ENABLE_PASSWORD_AUTH" == "yes" ]]; then
  echo "   Password auth:         ⚠ ENABLED (remember to disable after adding SSH key)"
  echo "   User password:         ${USER_PASS}"
  echo ""
  echo "   IMPORTANT: After first login, add your SSH key and disable password auth:"
  echo "     ssh-copy-id -p ${SSH_PORT} ${NEWUSER}@<SERVER_IP>"
  echo "     Then edit /etc/ssh/sshd_config: PasswordAuthentication no"
else
  echo "   Password auth:         NO (publickey only)"
  if [[ "$SHOW_PRIVATE_KEY" == "yes" ]]; then
    echo "   Private key:           Shown earlier (you must have saved it)"
  fi
fi
echo ""
